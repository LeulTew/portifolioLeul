...
describe('landing focus after navigation', () => {
  ...
  it('is not given way by that older navigation after a replay published in between', () => {
    // Round 16: a nested replay raised the count, and the outer navigation then read as newer.
    const stopOuter = subscribeSectionNavigation(target => {
      if (target !== 'about') return;
      publishSectionNavigation('projects', { source: 'navbar' });
      publishSectionNavigation('projects', { source: 'navbar', resume: true });
    });
    const stopInner = subscribeSectionNavigation((target, options) => {
      if (target === 'projects' && !options?.resume) landSectionFocus('projects');
    });
    publishSectionNavigation('about', { source: 'navbar' });
    stopOuter();
    stopInner();
    ...
    // A navigation begun after the landing still outranks it.
    byId('projects-link').focus();
    byId('stage').setAttribute('inert', '');
    landSectionFocus('projects');
    publishSectionNavigation('contact');
