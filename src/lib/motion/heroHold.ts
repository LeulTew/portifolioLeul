/**
 * How far through the hero's hold the reader is, shared by the few things that
 * need it.
 *
...
export function setHeroHold(progress: number): void {
...
export function getHeroHold(): number {
  return current;
}

export function subscribeHeroHold(listener: Listener): () => void {
...
export function resetHeroHold(): void {
  current = 0;
  listeners.clear();
}
